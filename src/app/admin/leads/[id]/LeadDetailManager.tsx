  ...
  return (
    <>
      ...
      <section className="panel">
        ...
        {enrollments.length === 0 ? <AdminEmptyState icon="courses" title="Sin inscripciones confirmadas" description="Este contacto puede tener un interés registrado, pero todavía no está inscrito." /> : <div className="table-wrap"><table className="data"><thead><tr><th>Curso</th><th>Situación</th><th>Finance</th><th>Origen</th></tr></thead><tbody>{enrollments.map((item) => <tr key={item.id}><td><a href={item.course.officialCourseUrl} target="_blank" rel="noreferrer">{item.course.title} ↗</a></td><td><span className={`status-dot ${item.status === "INSCRITO" || item.status === "COMPLETADO" ? "is-done" : "is-waiting"}`}>{SITUACION[item.status] ?? presentAdminValue(item.status)}</span></td><td>{item.financeStatus === "ENVIADO" && item.financeInscripcionId ? <span className="status-dot is-done">✓ Enviado a Finance</span> : item.financeStatus === "ERROR" ? <span className="status-dot is-waiting">No se pudo enviar a Finance</span> : item.financeStatus === "ENVIANDO" ? <span className="status-dot is-waiting">Enviando a Finance…</span> : <span className="muted">Pendiente de vinculación</span>}</td><td>{item.utmSource ?? item.source ?? "Orgánico"}{isTechnical && [item.utmCampaign, item.utmContent, item.utmTerm].some(Boolean) ? <div className="muted">{[item.utmCampaign, item.utmContent, item.utmTerm].filter(Boolean).join(" · ")}</div> : null}{item.landingUrl ? <a className="muted" href={item.landingUrl} target="_blank" rel="noreferrer">Ver origen ↗</a> : null}</td></tr>)}</tbody></table></div>}
      </section>
    </>
  );
}
