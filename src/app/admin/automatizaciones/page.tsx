...
export default async function AutomationsPage() {
  ...
  return <main className="container admin-shell">
    ...
    <AutomationManager
      ...
      rules={rules.map((rule) => ({ id: rule.id, courseId: rule.courseId, campaignId: rule.campaignId, name: rule.name, trigger: rule.trigger, offsetMinutes: rule.offsetMinutes, channel: rule.channel, subject: rule.subject, body: rule.body, status: rule.status, enrollmentStatuses: Array.isArray(rule.enrollmentStatuses) ? rule.enrollmentStatuses.filter((value): value is string => typeof value === "string") : [], course: rule.course.title, campaign: rule.campaign?.name ?? null, nextExecutionAt: rule.nextExecutionAt?.toISOString() ?? null, lastExecutedAt: rule.lastExecutedAt?.toISOString() ?? null, messages: rule._count.messages }))}
    />
  </main>;
}
