...
export default async function MessagesPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [messages, courses, templates] = await Promise.all([
  ...
  return <main className="container admin-shell">
    <AdminNav />
    <AdminPageHeader eyebrow="Comunicación" title="Seguimiento · mensajes" description="Supervisa los envíos programados, sus resultados y las plantillas de comunicación." actions={<DispatchButton simulation={isMessagingSimulation()} />} />
    ...
    <form><AdminFilterPanel><div className="filter-bar"><select name="channel" defaultValue={filters.channel ?? ""}><option value="">Todos los canales</option><option value="EMAIL">Correo</option><option value="WHATSAPP">WhatsApp</option></select><select name="status" defaultValue={filters.status ?? ""}><option value="">Todos los estados</option>{["PROGRAMADO","ENVIANDO","ENVIADO","SIMULADO","FALLIDO","CANCELADO","OMITIDO"].map((status) => <option key={status} value={status}>{presentAdminValue(status)}</option>)}</select><select name="course" defaultValue={filters.course ?? ""}><option value="">Todos los cursos</option>{courses.map((course) => <option value={course.id} key={course.id}>{course.title}</option>)}</select><input name="lead" defaultValue={filters.lead} placeholder="Buscar contacto" /><input name="from" type="date" defaultValue={filters.from} aria-label="Fecha desde" /><button type="submit" className="btn-sm">Filtrar</button><Link className="btn-sm ghost" href="/admin/mensajes">Limpiar</Link></div></AdminFilterPanel></form>
    ...
    <section className="panel"><h2>Plantillas</h2>{templates.length === 0 ? <AdminEmptyState icon="messages" title="Todavía no hay plantillas" description="Las plantillas administrables aparecerán aquí." /> : <div className="table-wrap"><table className="data"><thead><tr><th>Nombre</th><th>Canal</th><th>Categoría</th><th>Estado</th></tr></thead><tbody>{templates.map((template) => <tr key={template.id}><td>{template.name}</td><td>{presentAdminValue(template.channel)}</td><td>{template.category ?? "—"}</td><td><span className={`pill ${template.isActive ? "ok" : ""}`}>{template.isActive ? "Activa" : "Inactiva"}</span></td></tr>)}</tbody></table></div>}</section>
  </main>;
}
