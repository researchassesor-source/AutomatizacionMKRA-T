...
export default async function AuditPage({ searchParams }: { searchParams: Promise<{ action?: string; entity?: string; result?: string }> }) {
  ...
  return <main className="container admin-shell"><AdminNav view={view} /><AdminPageHeader eyebrow="Control interno" title="Auditoría" description="Revisa los eventos registrados para comprender quién hizo qué y con qué resultado." /><form><AdminFilterPanel label="Filtros de auditoría"><div className="filter-bar"><input name="action" defaultValue={filters.action} placeholder="Buscar acción" aria-label="Buscar acción" /><input name="entity" defaultValue={filters.entity} placeholder="Tipo de registro" aria-label="Tipo de registro" /><select name="result" defaultValue={filters.result ?? ""} aria-label="Resultado"><option value="">Todos los resultados</option><option value="SUCCESS">Correcto</option><option value="FAILURE">Fallido</option></select><button type="submit" className="btn-sm">Filtrar</button><Link className="btn-sm ghost" href="/admin/auditoria">Limpiar</Link></div></AdminFilterPanel></form><section className="panel">{logs.length === 0 ? <AdminEmptyState icon="audit" title="No hay eventos con estos filtros" description="Ajusta los criterios o limpia la búsqueda para ver otros registros." /> : <div className="table-wrap"><table className="data"><thead><tr><th>Fecha</th><th>Actor</th><th>Acción</th><th>Registro</th><th>Resultado</th><th title="Identificador interno del registro afectado. Sirve para localizarlo con exactitud.">Referencia</th><th>Detalle</th></tr></thead><tbody>{logs.map((log) => <tr key={log.id}><td>{new Intl.DateTimeFormat("es-EC", { dateStyle: "short", timeStyle: "medium", timeZone: "America/Guayaquil" }).format(log.createdAt)}</td><td>{log.actorEmail ?? "Sistema"}</td><td>{presentAuditAction(log.action)}<TechnicalOnly>{log.action}</TechnicalOnly></td><td>{presentAdminValue(log.entityType)}</td><td><span className={`pill ${log.result === "SUCCESS" ? "ok" : "err"}`}>{log.result === "SUCCESS" ? "Correcto" : "Fallido"}</span></td><td>{log.entityId ?? "—"}</td><td><details><summary>Ver evento</summary><dl className="detail-list"><dt>Quién</dt><dd>{log.actorEmail ?? "Sistema"}</dd><dt>Qué</dt><dd>{presentAuditAction(log.action)}</dd><dt>Cuándo</dt><dd>{new Intl.DateTimeFormat("es-EC", { dateStyle: "full", timeStyle: "medium", timeZone: "America/Guayaquil" }).format(log.createdAt)}</dd><dt>Entidad</dt><dd>{presentAdminValue(log.entityType)}</dd><dt title="Identificador interno del registro afectado.">Referencia</dt><dd>{log.entityId ?? "—"}</dd><dt>Resultado</dt><dd>{log.result === "SUCCESS" ? "Correcto" : "Fallido"}</dd><dt>Metadatos seguros</dt><dd><pre className="audit-metadata">{log.metadata ? JSON.stringify(log.metadata, null, 2) : "Sin metadatos"}</pre></dd></dl></details></td></tr>)}</tbody></table></div>}</section></main>;
}
